...
if (filenames.length === 0) {
  console.error(
    `No input files provided. Use 'npm run resolve --help' for more information.`,
  );
  process.exit(1);
}
...
function getFilesInDir(
  ...
  if (lsync.isFile()) {
    ...
    return files
      .map(file => getFilesInDir(path.join(dir, file), check))
...
if (files.length === 0) {
  console.error(
    `No files found in input directories or files: ${filenames.join(', ')}.`,
  );
  process.exit(1);
}
    ...
    return (phyxContent.phylorefs || []).map(phyloref => {
      ...
      function specifierToOTLId(specifier) {
        ...
        if (!wrappedSpecifier.taxonConcept) {
          ...
          const nameComplete = new phyx.TaxonConceptWrapper(specifier)
            .nameComplete;

          if (!nameComplete) {
            debug(
              `     - ${wrappedSpecifier.label} is missing a taxonomic name: ${JSON.stringify(specifier)}`,
            );
            return undefined;
          } else {
            const nameToUse = nameComplete.replace(
              /\s+\(originally \w+\)/g,
              '',
            );
            const { _, body } = retus(
            ...
            const _ottNames = matches
              .filter(match => match)
              .map(match => match.taxon.name)
              .filter(name => name);
            const ottIds = matches
              .filter(match => match)
              .map(match => match.taxon.ott_id)
              .filter(ott_id => ott_id);

            if (ottIds.length > 1)
              debug(
                `     - Taxon name ${nameComplete} resolved to multiple OTT Ids: ${ottIds.join(', ')}.`,
              );
      ...
      const internalOTTs =
        wrappedPhyloref.internalSpecifiers.map(specifierToOTLId);
      const internalOTTids = lodash.flattenDeep(
        internalOTTs.map(ott => lodash.head(lodash.values(ott))),
      );
      debug(`   - Internal specifiers: ${internalOTTids.join(', ')}`);
      // console.log(internalOTTids);

      const externalOTTs =
        wrappedPhyloref.externalSpecifiers.map(specifierToOTLId);
      const externalOTTids = lodash.flattenDeep(
        externalOTTs.map(ott => lodash.head(lodash.values(ott))),
      );
      ...
      if (internalOTTids.filter(x => x === undefined).length > 0) {
        debug(
          'Not all internal specifiers could be resolved to OTT Ids, skipping phyloreference.',
        );
        return {
          filename,
          phyloref,
        ...
        debug(
          'Not all external specifiers could be resolved to OTT Ids, skipping phyloreference.',
        );
        return {
          filename,
          phyloref,
        ...
        debug(
          'Cannot resolve phyloreference with a single internal specifier, skipping phyloreference.',
        );
        return {
          filename,
          phyloref,
        ...
        const result = retus(
          'https://api.opentreeoflife.org/v3/tree_of_life/mrca',
          {
            ...
            method: 'post',
            json: {
              node_ids: internalOTTids.map(id => `ott${id}`),
              excluded_node_ids: externalOTTids.map(id => `ott${id}`),
            },
            responseType: 'text',
          },
        );
        ...
        if (result.statusCode === 200) {
          ...
          debug(
            `   -> Could not find a MRCA: ${JSON.stringify(result, null, 2)}.`,
          );
          return {
            filename,
            phyloref,
          ...
          throw new Error(
            `Could not connect to the Open Tree of Life API: ${result}`,
          );
        }
        ...
        if (body.node_ids) {
          ...
          debug(
            `   -> ${body.node_ids.length} node IDs returned, with branch-based node at: https://tree.opentreeoflife.org/opentree/argus/${synth_id}@${node_id}`,
          );

          const nodeInfo = retus(
          ...
          let name = '';
          if (nodeInfo.body?.taxon?.unique_name) {
          ...
          debug(
            `   -> Found MRCA node (${name}): https://tree.opentreeoflife.org/opentree/argus/${synth_id}@${body.mrca.node_id}`,
          );
          return {
            filename,
            phyloref,
...
const results = lodash.groupBy(
  files
    .map(file => resolvePhyx(file))
    .reduce((acc, curr) => acc.concat(curr), [])
    .filter(x => x),
  result =>
    result.phyloref['@id'] ||
    new phyx.PhylorefWrapper(result.phyloref).label ||
    lodash.uniqueId('_'),
);
