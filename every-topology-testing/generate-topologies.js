...
normalizedUniqTrees.forEach((tree, index) => {
  ...
  /* We record all the phylogenies in a Phyx file for testing. */
  const phyx_document = {
  ...
  console.log(newick + "\t # " + summary);

  /* We also record some information for later export as Nexus. */
  ...
  labeledNewicks[index] = {
    index,
    newick: treeToNewick(tree),
    labeledNewick: newick,
    summary,
  };
});

/* Write out all trees to Nexus for validation. */
...
fs.writeFileSync(nexusFilename, nexusContent);
