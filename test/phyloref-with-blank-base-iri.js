...
// Which directory is this code working on?
const DIRNAME = path.resolve(__dirname, './examples/correct/with-blank-base-iri');
...
describe('PhyxWrapper', function () {
  ...
  describe('Test all correct example Phyx files', function () {
    const examples = fs.readdirSync(DIRNAME)
      .filter(filename => filename.endsWith('.json'));

    examples.forEach((example) => {
      const basename = path.resolve(DIRNAME, path.parse(example).name);
