...
describe(PHYX2OWL_JS, function () {
  it('should work without any arguments', function () {
    const result = child.spawnSync('node', [PHYX2OWL_JS], {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
  ...
  it('should support `--help`', function () {
    const result = child.spawnSync('node', [PHYX2OWL_JS, '--help'], {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
  ...
  it('should be able to convert `brochu_2003.json`', function () {
    ...
    const result = child.spawnSync('node', [PHYX2OWL_JS, PHYX_FILE, '--base-iri', 'http://example.org/phyx.js/example#'], {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
    expect(result.stderr).to.be.empty;
  ...
  it('should be able to convert the entire `test/examples/correct` directory', function () {
    ...
    const result = child.spawnSync('node', [PHYX2OWL_JS, EXAMPLE_DIR, '--base-iri', 'http://example.org/phyx.js/example#'], {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
    expect(result.status).to.equal(0);
  ...
  it('should give an error if no JSON files could be found', function () {
    const result = child.spawnSync('node', [PHYX2OWL_JS, __dirname], {
      encoding: 'utf-8',
      stdio: 'pipe',
    });
    expect(result.status).to.equal(1);
